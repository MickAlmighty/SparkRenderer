...
bool ImGuiFileBrowser::renderNavAndSearchBarRegion()
{
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.12f, 0.15f, 0.078f, 1.0f));
...
bool ImGuiFileBrowser::renderButtonsAndCheckboxRegion()
{
    ...
    else
    {
        if(ImGui::Button("Open"))
        {
            // It's possible for both to be true at once (user selected directory but input bar has some text. In this case we chose to open the
            // directory instead of opening the file. Also note that we don't need to access the selected file through "selected_idx" since the if a
            // file is selected, input bar will get populated with that name.
...
bool ImGuiFileBrowser::renderInputComboBox()
{
    ...
    if(show_inputbar_combobox && (ImGui::GetFocusedFocusScope() == focus_scope_id || ImGui::GetCurrentContext()->ActiveIdIsAlive == input_id))
    {
        ImGuiWindowFlags popupFlags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove |
                                      ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings;
    ...
    if(ImGui::BeginCombo("##FileTypes", selected_label))
    {
        ...
        for(std::vector<std::string>::size_type i = 0; i < valid_exts.size(); i++)
        {
            if(ImGui::Selectable(valid_exts[i].c_str(),
                                 selected_ext_idx == static_cast<int>(i) && !show_all_files && !show_files_with_valid_extensions))
            {
...
bool ImGuiFileBrowser::readDIR(std::string pathdir)
{
    ...
    /* If the current directory doesn't exist, and we are opening the dialog for the first time, reset to defaults to avoid looping of showing error
     * modal. An example case is when user closes the dialog in a folder. Then deletes the folder outside. On reopening the dialog the current path
     * (previous) would be invalid.
     */
    ...
    if(filter_mode | FilterMode_Files)
    {
        ...
        for(std::vector<Info>::size_type i = 0; i < subfiles.size(); ++i)
        {
            if(show_files_with_valid_extensions)
            {
                ...
                {
                    std::string ext =
                        subfiles[i].name.find_last_of('.') == std::string::npos ? "" : subfiles[i].name.substr(subfiles[i].name.find_last_of('.'));
...
bool ImGuiFileBrowser::validateFile()
{
    ...
    if(dialog_mode == DialogMode::SAVE)
    {
        // If file with extension matches, return false to show a replace file modal
        if(match)
        {
            return false;
        }
    ...
    // file doesn't exist so cant open/select)
    if(!match)
