...
{
    const auto& it = std::find_if(std::begin(processedNodes), std::end(processedNodes),
                                  [&node](const std::shared_ptr<NodeAI>& n) { return n->position == node->position; });
...
void ActorAI::walkToEndOfThePath()
{
    ...
    const auto wayPoint_it = std::find_if(path.begin(), path.end(), [](const std::pair<bool, glm::vec2>& p) { return !p.first; });
...
{
    const auto& it = std::find_if(std::begin(processedNodesStack), std::end(processedNodesStack),
                                  [&node](const NodeAI& n) { return n.position == node.position; });
